...
const analytics=await fetch(`${url}/dashboard/analytics?days=30`,{headers:{cookie}});assert.equal(analytics.status,200);const analyticsHtml=await analytics.text();assert.match(analyticsHtml,/Analytics de Empresa CI/);assert.match(analyticsHtml,/Avaliar/);const csvExport=await fetch(`${url}/api/analytics/export?days=30`,{headers:{cookie}});assert.equal(csvExport.status,200);assert.match(csvExport.headers.get("content-type")??"",/text\/csv/);assert.match(await csvExport.text(),/visualizacoes/);
