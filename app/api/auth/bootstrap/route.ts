...
export async function POST(request: Request) {
  ...
  if (
    ...
    parsed.data.bootstrapToken !== process.env.BOOTSTRAP_TOKEN
  )
    return Response.json(
      { error: "Inicialização não autorizada." },
      { status: 403 },
    );
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const existing = await tx
      ...
      .from(organizations)
      .where(eq(organizations.slug, parsed.data.organizationSlug))
      .limit(1);
  ...
  if (result === "already_initialized")
    return Response.json(
      { error: "Inicialização já concluída." },
      { status: 409 },
    );
  if (result === "slug_unavailable")
    return Response.json(
      { error: "Identificador de empresa indisponível." },
      { status: 409 },
    );
